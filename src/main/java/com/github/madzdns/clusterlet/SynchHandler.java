...
public class SynchHandler extends IoHandlerAdapter{
	...
	public SynchHandler synch(List<IMessage> msg) {
		...
		if(this.synch == SynchType.UNICAST ||
			...
			if(ids == null || ids.length == 0) {
				...
				for(Iterator<ClusterNode> it = cluster.iterator();
						it.hasNext();) {
					
					ClusterNode e = it.next();
				...
				if(synchModel) {
					...
					for(int i = 0 ; i< ids.length; i++) {
						...
						ClusterNode e = snapshot.getById(ids[i], 2); 
		...
		for(Iterator<IMessage> it = msg.iterator(); 
			...
			if(this.mode == SynchMode.SYNCH_MESSAGE) {
				...
				ClusterNode node = synchContext.getFrNodeById(cm.getId());
	...
	public void messageReceived(IoSession session, Object message)
		...
		/*
		 * If this Edge does not know the edge sent this
		...
		if(starter == false) {
			...
			ClusterNode him = (ClusterNode)session.getAttribute(HIM_KEY);
			...
			if(him != null && him.isDown()) {
				 ...
				 * TODO why DEL???????? 
				 */
				synchContext.synchronizedStateChange(him, ClusterNode.STATE_DEL);
			}
			...
			if(msg.getSynchMode() == SynchMode.SYNCH_MESSAGE) {
				...
				for(Iterator<SynchContent> it = contents.iterator(); 
					...
					IMessage decoded = (IMessage) this.encoder.newInstance();
				...
				if(messagesForRing.size() > 0) {
					...
					if(snapshot != null) {
						...
						List<ClusterNode> cluster = snapshot.getAliveCluster();
						
						if(cluster.size() > 0) {
							
							nodesForRingUpdate = new HashSet<Short>();
							
							for(Iterator<ClusterNode> it = cluster.iterator(); it.hasNext();) {
					...
					if(responses != null) {
						...
						for(Iterator<IMessage> respIt = responses.iterator(); respIt.hasNext();) {
							...
							if(response != null) {
								...
								ClusterNode node = synchContext.getFrNodeById(response.getId());
				...
				if(messagesForRing.size() > 0) {
					...
					while( mIt.hasNext()) {
						...
						if(s.isSuccessful()) {
							...
							ClusterNode node = synchContext.getFrNodeById(((ClusterMessage)m).getId());
			...
			if(msg.getSynchMode() == SynchMode.SYNCH_MESSAGE) {
				...
				for(Iterator<SynchContent> it = contents.iterator(); 
					...
					if(!result) {
						...
						ClusterNode node = synchContext.getFrNodeById(decoded.getId());
	...
	public void workCallback(SynchSession session, byte state, String link_ip) {
		...
		case UNICAST_ONE_OF:
			...
			if(state == STATE_WORK_FAILED || 
				...
				ClusterNode e = session.getFrNode();
				
				if(e != null && e.getState() == ClusterNode.STATE_DEL) {
					
					log.debug("Setting edge {} as DOWN",e.getId());
					
					synchContext.synchronizedStateChange(e, ClusterNode.STATE_DWN);
	...
	private void synchWithBalance(List<IMessage> msg) {
		...
		if(this.synch == SynchType.UNICAST_BALANCE ||
			...
			if(ids == null || ids.length == 0) {
				...
				for(Iterator<ClusterNode> eIt = cluster.iterator();
						eIt.hasNext();) {
					
					ClusterNode e = eIt.next();
			...
			Set<ClusterNode> edges = new HashSet<ClusterNode>();
